  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (r.ok && d.access) {
        ...
        onLogin();
      } else { setError(d.detail || 'Usuario o contraseña incorrectos'); }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-800 to-blue-900 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Contraseña" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none" required />
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-500 mb-2 font-semibold">Usuarios de prueba (click para autocompletar):</p>
          <div className="grid grid-cols-3 gap-2">
            { [['admin','admin123'], ['tecnico1','tecno123'], ['supervisor1','super123'] ].map(([u,p]) => (
              <button key={u} onClick={() => {setUsername(u); setPassword(p);}} className="text-xs bg-white border border-gray-200 rounded px-2 py-1 hover:border-blue-400 hover:text-blue-700 transition-colors">{u}</button>
            ))}
          </div>
        </div>
      </div>
...
function Diagnose() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      {step === 2 && box && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
          <div className="mb-6"><h2 className="text-xl font-bold text-gray-800">📦 {box.code} - {box.name}</h2><p className="text-gray-500 text-sm">{box.full_path}</p><p className="text-gray-400 text-sm">📍 {box.address}</p></div>
        ...
        <div className="space-y-4">
          ...
          {diagnosis.affected_clients_details?.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h3 className="font-bold text-gray-800 mb-3">👥 Clientes afectados ({diagnosis.affected_clients_details.length})</h3>
