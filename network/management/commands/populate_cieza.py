"""
FiberTruck - Comando para poblar la base de datos con datos ficticios
de un despliegue FTTH en Cieza, Murcia. Trabajo original para el TFM.
"""
import random
...
class Command(BaseCommand):
    help = 'Pobla la base de datos con el despliegue FTTH ficticio de Cieza, Murcia'

    def handle(self, *args, **options):
        self.stdout.write(self.style.MIGRATE_HEADING('=== FiberTruck: Despliegue FTTH de Cieza, Murcia ==='))
        ...
        for sd in SPLITTERS_DATA:
            ...
            self.stdout.write(f'  Splitter: {splitter}')
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\n{'='*50}\nDESPLIEGUE FTTH DE CIEZA - RESUMEN\n{'='*50}\n"
            f"OLT: 1 | Zonas: {len(zones_map)} | Splitters: {len(splitters_map)}\n"
            f"Cajas: {len(boxes_map)} | Clientes: {total_clients}\n{'='*50}"
        ))
